...
void * memsetAVX(void *dest, const uint8_t val, size_t numbytes)
{
  void * returnval = dest;
  ...
  else
  {
    ...
    if(val == 0)
    {
      if(numbytes > numbytes_to_align)
      {
        ...
        if((numbytes - numbytes_to_align) > CACHESIZE)
        {
...
void * memsetAVX_By4Bytes(void *dest, const uint32_t val, size_t numbytes_div_4)
{
  void * returnval = dest;

  if( ((uintptr_t)dest & BYTE_ALIGNMENT) == 0 ) // Check alignment
  {
    if((numbytes_div_4 * 4) > CACHESIZE)
    {
  ...
  else
  {
    ...
    if(numbytes_div_4 > (numbytes_to_align >> 2))
    {
      ...
      if((numbytes_div_4 * 4 - numbytes_to_align) > CACHESIZE)
      {
